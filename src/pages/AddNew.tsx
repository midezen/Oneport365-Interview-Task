...
const AddNew: React.FC = () => {
  ...
  return (
    <div id="container">
      ...
      <div
        id="container_top"
        className="flex justify-between items-center h-[118px] bg-[#FAFAFA] px-[25px] lg:px-[64px]"
      >
        <div id="container_topLeft" className="flex flex-col gap-[4px]">
          <Link to="/">
            <div
              id="container_topLeftTop"
              className="flex items-center font-normal text-[12px] text-[#6B7280] cursor-pointer ml-[-6px]"
            >
              <NavigateBeforeIcon className="text-[12px]" /> Back to quotes
            </div>
          </Link>
          <p className="text-[22px] sm:text-[24px] text-[#1F2937]-500">
        ...
        <div id="container_topRight" className="flex items-center gap-[24px]">
          <button className="py-[10px] px-[14px] rounded-[4px] bg-[#FFFFFF] border border-[#F3F4F6] text-[#6B7280] font-normal text-[14px]">
            Save <span className="hidden sm:flex">as</span> draft
          </button>
      ...
      <div id="container_body" className="px-[25px] lg:px-[64px] w-full ">
          ...
          {addNewSection
            ? quoteState.singleQuoteData.sections?.map((section) => {
                return (
                  <div
                    ...
                    className="flex md:flex-row gap-[24px] items-center mb-[30px] flex-col"
                    key={section._id}
                  >
                    <Section
                      data={section}
            ...
            : slicedSections?.map((section) => {
                return (
                  <div
                    id="container_bodySection"
                    className="flex gap-[24px] items-center mb-[30px] flex-col md:flex-row"
                    key={section._id}
                  >
                    <Section
                      data={section}
        ...
        <hr className="md:w-[73.5%] w-full mt-[43px] mb-[40px] " />
        <button
          onClick={() => setAddNewSection(true)}
          className="bg-[#37B2481A] h-[44px] w-full md:w-[73.5%] text-[14px] text-[#00861E] flex items-center gap-[8px] justify-center"
        >
        ...
        <hr className="w-full md:w-[73.5%] mt-[43px] mb-[40px] " />
        <div
          id="container_bodySectionsBottom"
          className="w-full md:w-[75%] flex justify-between mb-[30px]"
        >
          <Tooltip
